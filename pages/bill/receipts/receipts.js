...
Page({
  ...
  onShow: function () {
    ...
    this.setData({
      ...
      is_verify_cust_confirm: wx.getStorageSync('is_verify_cust_confirm'),//认证发票上传是否确认
      is_entrust: wx.getStorageSync('is_entrust'),//是否委托进项认证      
    })
  },
      ...
      success: function (sm) {
        if (sm.confirm) {
          ...
          util.request(api.BillApi.PostInvVerifyCustConfirm, confirmData, 'POST').then(function (res) {            
            if (res.data.success == true) {
              wx.setStorageSync('is_verify_cust_confirm','Y');
              self.setData({
